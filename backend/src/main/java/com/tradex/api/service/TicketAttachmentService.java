...
public class TicketAttachmentService {
    ...
    public AttachmentDownload getAttachmentDownload(String userEmail, Long attachmentId) {
        ...
        if (user.getRole() != Role.SUPER_ADMIN && user.getRole() != Role.EMPLOYEE
                && !attachment.getTicket().getUser().getId().equals(user.getId())) {
    ...
    @Transactional
    public void processAndAttachFiles(com.tradex.api.entity.SupportTicket ticket,
        ...
        for (org.springframework.web.multipart.MultipartFile file : files) {
            if (file != null && !file.isEmpty()) {
                if (file.getSize() > 5 * 1024 * 1024) {
                    throw new com.tradex.api.exception.AppException.BadRequestException(
                            "File size exceeds limit of 5MB: " + file.getOriginalFilename());
                }
                try {
                    com.tradex.api.util.ImageCompressionUtil.CompressionResult result = com.tradex.api.util.ImageCompressionUtil
                            .compressImage(file);
                    String effectiveFileName = com.tradex.api.util.ImageCompressionUtil
                            .getEffectiveFileName(file.getOriginalFilename(), result.contentType());

                    int dotIndex = effectiveFileName.lastIndexOf('.');
                    if (dotIndex > 0) {
                        effectiveFileName = effectiveFileName.substring(0, dotIndex) + "_" + System.currentTimeMillis()
                                + effectiveFileName.substring(dotIndex);
                    } else {
                    ...
                    throw new com.tradex.api.exception.AppException.BadRequestException(
                            "Could not process attachment: " + file.getOriginalFilename());
                }
            }
        }
    }
